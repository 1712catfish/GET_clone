...
class MatchInteraction(object):
    ...
    def __init__(self, data_pack: matchzoo.DataPack, **kargs):
        ...
        self.pos_queries, self.pos_docs, self.negatives, self.unique_queries_test = (
...
class ClassificationInteractions(BaseClassificationInteractions):
    ...
    def convert_text(self, ids, fixed_length, length, window_size, texts):

        texts = texts.split(" ")

        assert len(texts) == len([x for x in ids if x != 0])

        ids, texts = ids[:length], texts[:length]

        d = {}
        for id, text in zip(ids, texts):
            d[text] = id

        words_list = list(set(texts))

        words_list.sort(key=texts.index)
        words2id = {word: id for id, word in enumerate(words_list)}
        # words2id = add_synonyms(words2id)

        words2id = words_list.copy()
        for word in words2id:
            synonyms = get_synonyms(word)
            for synonym in synonyms:
                if synonym in words2id:
                    words2id[synonym] = words2id[word]


        length_ = len(words2id)
        neighbours = [set() for _ in range(length_)]
        # window_size = window_size if fixed_length == 30 else 300
        for i, (id, word) in enumerate(zip(ids, texts)):
            for j in range(max(i - window_size + 1, 0), min(i + window_size, length)):
                neighbours[words2id[word]].add(words2id[texts[j]])
...
def add_synonyms(words2id):
    new_words2id = words2id.copy()
    for word in list(words2id.keys()):
        synonyms = get_synonyms(word)
        for synonym in synonyms:
            if synonym in words2id:
                new_words2id[synonym] = words2id[word]
    return new_words2id


def get_synonyms(word):
    res = []
    for syn in wn.synonyms(word):
        res.extend(syn)
    return res
