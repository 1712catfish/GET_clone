...
from torch_geometric.nn.norm import LayerNorm, MessageNorm, GraphNorm
...
class GGNN_with_GSL(nn.Module):
    ...
    def forward(self, adj, feat):
        ...
        # TODO: return adj_refined
        return feat, adj_refined

class GGNN_GSL_2(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, rate=0.8, dropout=0.2):
        super().__init__()

        self.feat_prop1 = GGNN(input_dim, hidden_dim, dropout)

        self.gatconv = GATv2Conv(hidden_dim, hidden_dim, heads=1)
    ...
    def forward(self, adj, feat):
        ...
        res = []
        for nodes, ads in zip(feat, adj):

            edges = ads.nonzero().t().to(torch.int64).to("cuda:6")

            nodes = self.gatconv(nodes, edges)

            # nodes = self.norm(nodes)

            # nodes = nn.LeakyReLU()(nodes)
...
class Tunneling(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, rate=0.8, dropout=0.2):
        super().__init__()

        self.feat_prop1 = GGNN(input_dim, hidden_dim, dropout)

        self.gatconv = GATv2Conv(-1, hidden_dim, heads=1)
        # self.norm = LayerNorm(hidden_dim)
...
class GSL(nn.Module):
    ...
    def forward(self, adj, score):
        ...
        mask = torch.zeros([BATCH_SIZE, N, N]).cuda("cuda:6")
