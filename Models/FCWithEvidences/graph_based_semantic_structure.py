...
from Models.BiDAF.wrapper import *
from thirdparty.two_branches_attention import *
import numpy as np
import torch_utils as my_utils
import torch.nn as nn
from torch_geometric.nn.conv import SimpleConv, APPNP
...
class Graph_basedSemantiStructure(BasicFCModel):
    ...
    def __init__(self, params):
        ...
        #     input_dim=self.hidden_size, hidden_dim=96,
        #     output_dim=self.hidden_size, rate=self.gsl_rate,
        #     dropout=self.dropout_gnn
        # )

        # self.lin1 = nn.LazyLinear(D, self.hidden_size)

        # # TODO: Mul Mask



        # # self.ggnn_gsl1 = GGNN_with_GSL(input_dim=self.hidden_size, hidden_dim=self.hidden_size, output_dim=self.hidden_size, rate=self.gsl_rate, dropout=self.dropout_gnn)

        # self.lin2 = nn.Linear(self.hidden_size, D)
        # self.bn = nn.BatchNorm1d(100)

        # self.relu = nn.ReLU()

        # self.gconv = SimpleConv("mean")

        # self.norm1 = GraphNorm(hs)

        # # self.appnp = APPNP(30, 0.5)

        # self.gconv = SimpleConv("mean")

        # self.norm2 = GraphNorm(hs)
        # self.lin2 = nn.LazyLinear(hs * 4)
        # self.hswish = nn.Hardswish()

        # self.norm3 = GraphNorm(hs)

        # self.sigmoid = nn.Sigmoid()



        # self.pool = pool.SAGPooling(D, min_score=0.5)

        self.gconv = SimpleConv("max")
        hs = D
        self.li1 = nn.LazyLinear(hs)
        self.li2 = nn.LazyLinear(hs)
        self.o_proj = nn.LazyLinear(hs)
        self.no = nn.LayerNorm(hs)
    ...
    def forward(
        ...
        # ggnn for doc
        embed_doc, _ = self.ggnn_with_gsl(doc_adj, embed_doc)
        ...
        if True:
            ...
            for _ in range(4):
                ...
                res = []
                for nodes, adj in zip(x, doc_adj):
                    edges = adj.nonzero().t().to(torch.int64).to("cuda:6")

                    for _ in range(20):
                        nodes = self.gconv(nodes, edges)
            ...
            heads = heads.flatten(2, 3)
            heads = self.o_proj(heads)

            embed_doc = heads

        print(embed_doc.shape)
        print(query_repr.shape)

        # Step 1: word-level attention
        avg, word_att_weights = self._word_level_attention(
            left_tsr=query_repr, right_tsr=embed_doc, right_mask=doc_mask, **kargs
        )
